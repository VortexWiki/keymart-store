...
async function run() {
  ...
  // Fetch types avec concurrence limitée
  const CONCURRENCY = 5;
  ...
  for (const cat of sortedCategories) {
    ...
    for (const item of groups[cat]) {
      ...
      tableRows += `
      <tr class="item-row" data-name="${item.name.toLowerCase()}" data-cat="${cat}">
        <td class="td-check"><input type="checkbox" class="item-check" data-id="${item.id}" data-name="${item.name}" data-qty="${item.qty}"></td>
        <td class="td-icon"><img src="${item.icon}" alt="${item.name}" class="item-icon" onerror="this.style.display='none'"></td>
        <td class="td-name"><a href="https://sovereignro.com/?module=item&action=view&id=${item.id}" target="_blank">${item.name}</a></td>
        <td class="td-qty"><span class="qty-badge ${qtyClass}">×${item.qty}</span></td>
        <td class="td-spinner">
          <div class="spinner">
            <button class="spin-btn" onclick="changeQty('${item.id}', -1)">−</button>
            <span class="spin-val" id="spin-${item.id}">1</span>
            <button class="spin-btn" onclick="changeQty('${item.id}', 1, ${item.qty})">+</button>
          </div>
        </td>
        <td class="td-offer"><input type="text" class="offer-input" placeholder="offer / total (z)" data-id="${item.id}"></td>
        <td class="td-copy"><button class="copy-btn" onclick="copyItem('${item.id}','${safeName}',${item.qty})">📋</button></td>
      </tr>`;
    }
  }
...
<body>
  ...
  <div class="intro">
    ...
    <p>Example: <code>${DISCORD_USERNAME} Crimson Revolver ×2 — offer: 50,000z total</code></p>
  </div>
  <div class="stats">
  ...
  <footer>Key-Mart' Store · SovereignRO · ${dateStr}</footer>
  ...
  <script>
    ...
    function copyItem(id, name, maxQty) {
      ...
      if (!offer) {
        offerEl.classList.add('error');
        offerEl.focus();
        showToast('⚠️ Please enter an offer first', true);
        return;
      }
      offerEl.classList.remove('error');
      const qty = getSelectedQty(id);
      const text = DISCORD_USER + ' ' + name + ' ×' + qty + ' — offer: ' + offer + 'z total';
    ...
    function copyWishlist() {
      ...
      if (checked.length === 0) { showToast('⚠️ No items selected', true); return; }
      let hasError = false;
      ...
      if (hasError) { showToast('⚠️ Please fill in all offers', true); return; }
      ...
      for (const cb of checked) {
        ...
        msg += '- ' + cb.dataset.name + ' ×' + qty + ' — offer: ' + offer + 'z total\\n';
      }
